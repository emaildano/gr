  ...
  $.fn.inView = function(options) {
    ...
    function fade_in() {
      $(selector).each( function(i){
        ...
        // var elTop = $this.offset().top + opts.triggerOffset; // + $(this).outerHeight();
        // var winBottom = $(window).scrollTop() + $(window).height();
...
function filter_toggle_class_toggle() {
  $('body, #nav-filter, #js--filter--link-wrapper').toggleClass('open-filter');
  ...
  front_page: {
    ...
    finalize: function() {
      ...
      $(window).load(function(){
        setTimeout(function(){
          document.getElementsByTagName('body')[0].className+=' logo-animate';
        }, 500);
      });
    }
  },
  clients_archive: {
    init: function() {
      ...
      $('.client-fliter--link').click(function(event){
        ...
        $('.client-list--entry, .client-fliter--link').removeClass('active').addClass('inactive');
        ...
        $('#j2--client-toggle span').text( $this.text() );
  ...
  projects_archive: {
    init: function() {
      ...
      function removeMods(elWrapper, el, vaultWrapper) {
        elWrapper.find(el).not($wrapperInit).remove().clone().appendTo(vaultWrapper).addClass('removing');
      }
      ...
      if( wrapperInit ) {
        ...
        removeMods($archive, '.project-archive-module', '#js--filter-archive-vault');
      ...
      $('.js--filter-projects--link').click(function(event){
        ...
        $wrapper.find('.project-module').not($target).addClass('removing');
        $archive.find('.project-archive-module').not($target).addClass('removing');

        setTimeout(function() {
          $wrapper.find('.removing').remove().clone().appendTo('#js--filter-vault');
          $archive.find('.removing').remove().clone().appendTo('#js--filter-archive-vault');
        }, 350);

        // Get relevant modules out of the vault and move them to the main container
        setTimeout(function() {
          $vault.find('.' + target).remove().clone().appendTo('#js--filter-container');
          $archiveVault.find('.' + target).remove().clone().appendTo('#js--filter-archive');
          $wrapper.find('.project-module').sort(sort_mods).appendTo('#js--filter-container'); // sort elements
        }, 350);
  ...
  page_contact: {
    init: function() {
      ...
      geocoder.geocode( { 'address': address}, function(results, status) {
        ...
        var plusMarker = {
          path: 'M13.6, 13.6V0h2.7v13.6H30v2.7H16.4V30h-2.7V16.4H0v-2.7H13.6 z',
          fillColor: '#00bdd4',
          fillOpacity: 0.8,
          scale: 0.5,
          strokeColor: 'gold',
